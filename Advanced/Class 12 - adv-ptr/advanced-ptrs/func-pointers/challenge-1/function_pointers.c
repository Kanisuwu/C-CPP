...
int (*farray[])() = { &add, &sub, &mult, &divi };

// performs the given operation on elements of the arrays
int* performOp(int *a, int *b, int size, int (*farray)(int, int)); /* ADD FUNCTION POINTER PARAMETER HERE */
...
int main( ){
  ...
  while(choice != 5)
  {
	...
	result = performOp(array1,array2,size, farray[j]);		
...
int* performOp(int *a, int *b, int size, int (*farray)(int, int))
{
      int *array_ptr;
      array_ptr = (int *)malloc(size * sizeof(int));
      for (int i = 0; i < size; i++) {
          array_ptr[i] = farray(*(a++), *(b++));
      }
